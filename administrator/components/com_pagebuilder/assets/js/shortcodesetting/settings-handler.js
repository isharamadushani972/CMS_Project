    ...
    $.ShortcodeSetting.setTinyMCE = function (selector) {
        $(selector).each(function () {
            ...
            if (current_id) {
				...
				var validElements = undefined;

	            currentElement.attr('data-new-id', new_id);
	            currentElement.attr('id', new_id);
	            currentElement.addClass(new_id);

                if ($(this).hasClass('tinymce'))
                {
                	// Get settings to init TinyMCE.
                	var settings = document.querySelector('script.joomla-script-options');

                	if (settings) {
                		settings = JSON.parse(settings.textContent);
                	}

					try
					{
    					validElements = settings.plg_editor_tinymce;
					}
					catch(err)
					{
                    ...
                    tinymce.init({
                        ...
                        entity_encoding : "raw",
                        valid_elements : typeof validElements != 'undefined' ? validElements.tinyMCE['default'].valid_elements : '',
                        extended_valid_elements : typeof validElements != 'undefined' ? validElements.tinyMCE['default'].extended_valid_elements : 'iframe|hr[id|title|alt|class|width|size|noshade]',
                        force_br_newlines : false, force_p_newlines : true, forced_root_block : 'p',
                        toolbar_items_size: "small",
                        invalid_elements : 'script,applet',
                        // Plugins
                        plugins : "autolink,lists,save,colorpicker,importcss,searchreplace,link,anchor,image,code,textcolor,table,charmap,hr,directionality,paste,visualchars,visualblocks,nonbreaking,insertdatetime,wordcount,advlist,autosave,contextmenu",
                        // Toolbar
                ...
                else if ($(this).hasClass('jce'))
                {
                    ...
                    if(version == "new")
                    {
                        var editor = WFEditor._getEditor(this);
                        ...
                        if (editor)
                        {
                            ...
                            if (tinyMCE.editors.hasOwnProperty(current_id))
                            {
                                delete tinyMCE.editors[current_id];
                            }
                        }
                    ...
                    WFEditor.init({
                        ...
                        etag: etag,
                        theme: "advanced",
                        plugins: "autolink,cleanup,core,source,code,colorpicker,upload,format,charmap,contextmenu,browser,inlinepopups,media,clipboard,searchreplace,directionality,fullscreen,table,textcase,print,style,nonbreaking,visualchars,visualblocks,xhtmlxtras,imgmanager,anchor,link,spellchecker,article,lists,formatselect,styleselect,fontselect,fontsizeselect,fontcolor,importcss,advlist,wordcount",
                        language_load: false,
                        ...
                        theme_advanced_buttons3: "ltr,rtl,search,replace,|,table_insert,delete_table,row_props,cell_props,row_before,row_after,delete_row,col_before,col_after,delete_col,split_cells,merge_cells",
                        ...
                        formatselect_blockformats: formatselect,
                        fontselect_fonts: typeof jceDataFontSelect != "undefined" ? jceDataFontSelect : "Andale Mono=andale mono,times;Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Book Antiqua=book antiqua,palatino;Comic Sans MS=comic sans ms,sans-serif;Courier New=courier new,courier;Georgia=georgia,palatino;Helvetica=helvetica;Impact=impact,chicago;Symbol=symbol;Tahoma=tahoma,arial,helvetica,sans-serif;Terminal=terminal,monaco;Times New Roman=times new roman,times;Trebuchet MS=trebuchet ms,geneva;Verdana=verdana,geneva;Webdings=webdings;Wingdings=wingdings,zapf dingbats",
            ...
            if (action_type && action) {
                if (action_type == 'convert') {
                    ...
                    if (from && to) {
	                    ...
	                    var isEdit = $('.jsn-element.active-shortcode').length > 0 ? true : false;

                        // Close edit settings modal.
                        jParent('.ui-dialog #close').trigger('click');

                        if (shortcode_content) {
                            $('body').trigger('on_after_convert', [isEdit, 'pb_' + to, shortcode_content, modalParams]);
                        }
                    }
                }
            }
