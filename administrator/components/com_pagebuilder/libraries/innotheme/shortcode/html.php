...
if ( ! class_exists( 'IG_Pb_Helper_Html' ) ) {
	...
	class IG_Pb_Helper_Html {
		...
		static function tiny_mce($element) {
			...
			{
				$fontsString = 'Andale Mono=andale mono,times;Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Book Antiqua=book antiqua,palatino;Comic Sans MS=comic sans ms,sans-serif;Courier New=courier new,courier;Georgia=georgia,palatino;Helvetica=helvetica;Impact=impact,chicago;Symbol=symbol;Tahoma=tahoma,arial,helvetica,sans-serif;Terminal=terminal,monaco;Times New Roman=times new roman,times;Trebuchet MS=trebuchet ms,geneva;Verdana=verdana,geneva;Webdings=webdings;Wingdings=wingdings,zapf dingbats';
				...
				if (is_file(JPATH_ROOT . '/components/com_jce/editor/libraries/classes/utility.php'))
				{
					include_once JPATH_ROOT . '/components/com_jce/editor/libraries/classes/utility.php';
				}

				if (is_file(JPATH_ROOT . '/components/com_jce/editor/libraries/classes/application.php'))
				{
					include_once JPATH_ROOT . '/components/com_jce/editor/libraries/classes/application.php';
				}

				if (is_file(JPATH_ROOT . '/components/com_jce/editor/libraries/classes/language.php'))
				{
					include_once JPATH_ROOT . '/components/com_jce/editor/libraries/classes/language.php';
				}

				if (is_file(JPATH_ADMINISTRATOR . '/components/com_jce/helpers/plugins.php'))
				{
					include_once JPATH_ADMINISTRATOR . '/components/com_jce/helpers/plugins.php';
				}

				if (is_file(JPATH_ROOT . '/components/com_jce/editor/libraries/classes/editor.php'))
				{
					include_once JPATH_ROOT . '/components/com_jce/editor/libraries/classes/editor.php';
				}
                ...
                // create token
				if (class_exists('WFToken') && method_exists('WFToken', 'getToken'))
				{
					$token = WFToken::getToken();
				}
				elseif (method_exists('JSession', 'getFormToken'))
				{
                ...
                if (defined('WF_VERSION') && version_compare(WF_VERSION, '2.6.0') >= 0)
                {
                    $jceData .= " data-version=new";
                }
                else
                {
                    $jceData .= " data-version=old";
                }
                // toolbar theme
				if (method_exists('WFEditor', 'getParam'))
				{
					if (method_exists('WFEditor', 'getInstance'))
					{
				...
				elseif (method_exists('WFApplication', 'getParam'))
				{
					$wf = WFApplication::getInstance();
				}

                $fontSizes = $wf->getParam('fontsizeselect.font_sizes', '8pt,10pt,12pt,14pt,18pt,24pt,36pt','8pt,10pt,12pt,14pt,18pt,24pt,36pt');
                $fontSelect = $wf->getParam('fontselect.fonts', 'Andale Mono=andale mono,times;Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Book Antiqua=book antiqua,palatino;Comic Sans MS=comic sans ms,sans-serif;Courier New=courier new,courier;Georgia=georgia,palatino;Helvetica=helvetica;Impact=impact,chicago;Symbol=symbol;Tahoma=tahoma,arial,helvetica,sans-serif;Terminal=terminal,monaco;Times New Roman=times new roman,times;Trebuchet MS=trebuchet ms,geneva;Verdana=verdana,geneva;Webdings=webdings;Wingdings=wingdings,zapf dingbats');
                if ($fontSizes == '')
                {
              ...
              //  $jceData .= " data-font_select=" . $fontSizes;
              	if ($fontSizes != '')
              	{
	                $fonts = json_decode($fontSelect);
					if (count($fonts))
					{
		              	foreach ($fonts as $key => $item)
		              	{
		              		$fontsStringArray[] = $key . '=' . $item;
		              	}

		              	if (count($fontsStringArray))
		              	{
		              		$fontsString = implode(';', $fontsStringArray);
		              	}
					}
              	}

                $script .="<script type='text/javascript'>var jceDataFontSelect='" . $fontsString  . "';</script>";
                // get current component
                $option = JFactory::getApplication()->input->get('option');

                if (class_exists('WFExtensionHelper') && method_exists('WFExtensionHelper', 'getComponent'))
                {
			...
			$output  = $script. "<textarea type='$type' class='{$element['class']} jsn_tiny_mce {$role} {$editor}' id='{$element['id']}' name='{$element['id']}' {$role2} {$text_direction} {$urlbase} {$langPrefix} {$jceData} DATA_INFO>{$element['std']}</textarea>";
