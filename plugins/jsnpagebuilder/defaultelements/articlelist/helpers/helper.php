...
class JSNPbArticleListHelper
{
        ...
        switch ($attributes['articlelist_source']) {
            case self::PB_ARTICLE_SOURCE_K2 :
                ...
                foreach ($result as $_key => $_value) {
                    ...
                    $articles[$_key]['direct_url'] = JRoute::_(K2HelperRoute::getItemRoute($_value['id'].':'.urlencode($_value['alias']), $_value['catid'].':'.urlencode($_value['categoryalias'])));
                    $articles[$_key]['category_direct_url'] = JRoute::_(K2HelperRoute::getCategoryRoute($_value['catid'].':'.urlencode($_value['categoryalias'])));
            ...
            case self::PB_ARTICLE_SOURCE_EASY :
                ...
                foreach ($result as $_key => $_value) {
					...
					{
						$articles[$_key]['direct_url'] = JRoute::_('index.php?option=com_easyblog&view=entry&id=' . $_value['id']);
						$articles[$_key]['category_direct_url'] = JRoute::_('index.php?option=com_easyblog&view=categories&layout=listings&id=' . $_value['category_id']);						
					}
                    if ($attributes['is_old_version']) {
    ...
    public static function wordLimiter($str, $limit = 100, $end_char = '&#8230;')
    {
		...
		$matches[0] = str_replace('$', '&dollar;', $matches[0]);
