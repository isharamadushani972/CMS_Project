...
class JSNISFlowDisplay extends JObject
{
	...
	public function standardLayout($args)
	{
		...
		// Fix Jomres conflict
		$isInstalledJomres = $this->isInstalledComponent('com_jomres');
		if((int) $isInstalledJomres)
		{
			$document->addScriptDeclaration('
					if (typeof jQuery.ui != "undefined")
					{
						if (jQuery.ui.version != "1.8.9")
						{
							var jsnisflowscript = document.createElement("script");
							jsnisflowscript.setAttribute("type", "text/javascript");
							jsnisflowscript.setAttribute("src", "'. JUri::root() .$this->_assetsPath.'js/jquery-ui-1.8.9.custom.js");
							document.getElementsByTagName("head")[0].appendChild(jsnisflowscript);
						}
					}
			');
		}
	...
	function isInstalledComponent($name)
	{
		$db 	= JFactory::getDbo();
